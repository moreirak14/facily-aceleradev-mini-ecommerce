...
def create_token(data: dict, expire_delta=None):
    ...
    if expire_delta:
        ...
        )  # --> pega a data agora e soma com expire_delta
    else:
        expire = datetime.utcnow() + timedelta(
            minutes=60
        )  # --> ou... por padrão será 60 min

    payload.update({"exp": expire})

    return jwt.encode(
        payload, JWT_SECRET, ALGORITHM
    )  # --> modelo de criptografia HASH 256
...
def get_user(
    token: str = Depends(oauth2_schema), user_repository: UsersRepository = Depends()
):
    try:
